...
class User extends CI_Controller
{
    ...
    {
        $data['title'] = 'Dashboard Event';
        // ambil data user dari database berdasarkan email dari session
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();

        $this->db->where('user_id', $data['user']['id']);
        $this->db->where('subscribed >', 0);
        $this->db->from('user_event');
        $data['subscribed']  = $this->db->count_all_results();
        ...
        while ($dateplus != $enddate) {
            ...
            $this->db->where('date_start', $dateplus);
            $this->db->where('subscribed >', 0);
            $this->db->from('user_event');
            $count_event_day = $this->db->count_all_results();

            $data['event_day'] =  $data['event_day'] + $count_event_day;
            ...
            $query = "SELECT `user`.`email` , `user_event`.*
                   ...
                   WHERE `user_subscribe`.`vendor_id` = '$temp' AND `user_event`.`date_start` = '$dateplus'               
            ";
            $data['broad'] = $this->db->query($query)->result_array();
            if ($data['broad']) {
                foreach ($data['broad'] as $b) {
                    $count++;
                }
            }
            if ($data['broad']) {
        ...
        $data['count_not_broadcast']  = $countday;
        $data['count_need_broadcast']  = $count;

        // **********************************************
        // **********************************************
        // **********************************************

        $data['broadcast'] = $this->db->get_where('user_broadcast', ['date_created' => date('Y-m-d', time()), 'vendor_id' => $user_id])->row_array();
        if (!$data['broadcast']) {
            $data['broadcast']['count'] = 0;
        }

        if ($data['count_not_broadcast'] > 0) {
            // send pesan
    ...
    public function broadcastevent()
    {
        ...
        while ($dateplus != $enddate) {
            ...
            if ($data['broad']) {
                ...
                // -1 month
                $datemin = date('Y-m-d', strtotime("-1 month", strtotime($dateplus)));
                $data = [
                    // 'date_created' => date('Y-m-d', time()),
                    'date_created' => $datemin,
                    'vendor_id' => $user_id,
                    'count' => $i,
                ];
    ...
    private function _sendEmail($token, $type, $br)
    {
        ...
        if ($type == 'broadcast') {
            ...
            $message = '
                <table border="0">
                    <tr>
                        <th align="left">Dont miss it !!</th>                        
                    </tr>                    
                    <br>
                    <tr>
                        <th colspan="2" align="left"> ' . $br['name'] . ' by ' . $br['user_name'] . ' </th>
                    </tr>
                    <tr>
                        <td colspan="2" align="left"> ' . date('d F Y', strtotime($br['date_start'])) . ' - ' . date('d F Y', strtotime($br['date_end'])) . ' </td>
                    </tr>
                    <tr>
                        <td colspan="2" align="left"> ' . '<a href="' . base_url() . 'front/detailevent/' . $br['id'] . '"> Click Here for Details </a>' . ' </td>
                    </tr>
